...
This document covers configuration standards for VS Code-based editors used in this ecosystem. A VS Code fork is the typical editor, and these conventions apply to any fork of VS Code (Cursor, Windsurf, etc.) because they share the same settings schema and config directory structure.

## Config File Locations

| Editor | Settings Path |
| :--- | :--- |
| **VS Code fork** | `~/Library/Application Support/Antigravity/User/` |
...
This association is engine-level — it applies in VS Code and any VS Code fork without modification because the `vscode.markdown.preview.editor` command is built into the VS Code base.
...
## Baseline `settings.json` for VS Code forks
...
- **Do NOT assume VS Code settings paths apply to forks.** Always use the fork-specific path when reading or writing config files.
